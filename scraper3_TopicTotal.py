...
#labelling for Word2Vec neural network
def labellingForWord2Vec(text):
    #key words which will be used to select posts for training set
    key_words = {'depressed','hopeless','despair','self-conscious','suicidal'}
...
def getPageForWord2Vec(aPage):
    global posts, identified_posts
    
    for post in aPage.select('.post'):
        posts += 1

        #Parse & break apart object
        nick = post.select('.nick')[0].getText()
        time = post.select('.post_time')[0].getText()
        text = post.select('p')[0].getText()
        ...
        if labellingForWord2Vec(textMapped):
            identified_posts += 1
            if identified_posts % 100 == 0 and identified_posts > 0:
                print(identified_posts,"posts found for training set.") 
            outputWriter.writerow([nick,time,text])
...
topicLink = 'https://www.mumsnet.com/Talk/' + input("Enter the topic: ")
